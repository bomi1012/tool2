...
public class ChangeKategoryBean extends TemplateBuilder implements View, ValueChangeListener, IChangeViewPage {
	...
	private void componetsManager() {
		...
		m_nameField = textField(TEXT_FIELD_KATEGORIE_NAME, WIDTH_FULL, true, TEXT_FIELD_KATEGORIE_NAME, 45);
	...
	private void clickListener() {
		m_buttonSpeichern.addClickListener(new ClickListener() {
			...
			private void saveItem() {
				...
				try {
					...
					message(MESSAGE_SUSSEFULL_ARG_1, "Kategorie");	
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
		...
		m_buttonDeaktiviren.addClickListener(new ClickListener() {
			@Override
			public void buttonClick(ClickEvent event) {
				windowModalYesNoRemove(m_kategorie);	
			}
		});
	}
	...
	private void changeListener() {
		m_nameField.addValueChangeListener(new ValueChangeListener() {		
			...
			public void valueChange(ValueChangeEvent event) {
				m_kategorie.setName(String.valueOf(event.getProperty().getValue()));				
			}
		});
	}
	...
	private void close() {
		if (ChangeKategoryBean.this.getParent() instanceof Window) {					
			((Window) ChangeKategoryBean.this.getParent()).close();
		} else {
	...
	private void setNewInfo() {
		getData().clear();
		getData().put(m_nameField, m_kategorie.getName());
		
		m_toCreate = false;		
		m_headLine.setValue("Kategorie bearbeiten");
